...
function preparationArrivalState(kind, resumed) {
  const isDrink = kind === 'drink';
  const isBatch = kind === 'batch';
  const station = isDrink
    ? { id: 'dispenser', equipmentId: null, x: 100, y: 100 }
    : { id: 'k1', equipmentId: 'eq1', x: 100, y: 100 };
  const customers = isDrink
  ...
  const itemIds = isBatch ? ['i1', 'i2'] : ['i1'];
  const serviceItems = itemIds.map((id, index) => ({
    id,
    kind: isDrink ? 'drink' : 'dish',
    menuItemId: isDrink ? 'water' : 'starter-toast',
    customerId: `c${index + 1}`,
    tableId: `t${index + 1}`,
    state: resumed ? 'preparing' : 'ordered',
    stationId: station.id,
    assignedStaffId: 'cook',
    ...(isDrink ? { serviceTableId: 'st1', serviceSlotIndex: 0 } : {}),
    ...(isBatch ? { batchId: 'batch' } : {}),
    ...(resumed ? {
      preparationStartedAt: 0,
      accumulatedWork: 20 + index,
      lastProgressAt: 0,
    } : {
  ...
  return isolatedRunState({
    ...
    serviceTables: [{ id: 'st1', x: 300, y: 100 }],
    customers,
    dishes: isDrink ? [] : [{
      id: 'starter-toast', prepTime: 60, requiredEquipmentId: 'eq1', quality: 1,
    }],
    equipment: isDrink ? [] : [{ id: 'eq1', owned: true, speedMultiplier: 1 }],
    staff: [{
      id: 'cook', role: 'cook', skill: isBatch ? 5 : 3, morale: 80,
      x: 70, y: 110,
      task: {
        type: isDrink ? 'prepare_drink' : 'prepare_dish',
        serviceItemId: 'i1',
        ...(isDrink ? {
          stationId: station.id, serviceTableId: 'st1', serviceSlotIndex: 0,
        } : {
          stationId: station.id,
          ...(isBatch ? { batchId: 'batch', serviceItemIds: itemIds } : {}),
        }),
      },
    ...
    ...(isBatch ? {
      cookingBatches: [{
        id: 'batch', cookId: 'cook', stationId: station.id, serviceItemIds: itemIds,
...
function runDeterministicMovementSequence(reverseEntries) {
  let state = {
    ...
    customers: [{
      id: 'customer', state: 'eating', x: 100, y: 180,
...
describe('runTick', () => {
  ...
  it.each(['drink', 'dish', 'batch'])
    ('does not credit walking time when a resumed %s preparation arrives during a long tick', kind => {
      const initial = preparationArrivalState(kind, true);
      const before = initial.serviceItems.map(item => ({
        id: item.id, accumulatedWork: item.accumulatedWork, lastProgressAt: item.lastProgressAt,
      }));

      const arrived = runTick(initial, { gameDt: 10, movementDt: 1 });
      const arrivedItems = arrived.serviceItems.filter(item => before.some(entry => entry.id === item.id));

      arrivedItems.forEach(item => {
        const previous = before.find(entry => entry.id === item.id);
        expect(item.accumulatedWork).toBe(previous.accumulatedWork);
        expect(item.lastProgressAt).toBe(10);
      });

      const stationary = runTick(arrived, { gameDt: 1, movementDt: 0 });
      stationary.serviceItems
        .filter(item => before.some(entry => entry.id === item.id))
        .forEach(item => {
          const previous = arrivedItems.find(entry => entry.id === item.id);
          expect(item.accumulatedWork).toBeGreaterThan(previous.accumulatedWork);
          expect(item.lastProgressAt).toBe(11);
        });
    });

  it.each(['drink', 'dish', 'batch'])
    ('starts an initial %s preparation at arrival without retroactive work', kind => {
      const arrived = runTick(preparationArrivalState(kind, false), { gameDt: 10, movementDt: 1 });

      arrived.serviceItems.forEach(item => {
        expect(item.state).toBe('preparing');
        expect(item.accumulatedWork).toBe(0);
        expect(item.lastProgressAt).toBe(10);
      });
    });
  ...
  it('moves a drink from dispenser preparation to its counter slot before waiter pickup is possible', () => {
    let state = isolatedRunState({
      ...
      staff: [
        { id: 'cook', role: 'cook', skill: 3, morale: 80, x: 600, y: 300, task: null },
        { id: 'waiter', role: 'waiter', skill: 3, morale: 80, x: 500, y: 300, task: null },
      ],
      serviceItems: [{
    ...
    let sawOnService = false;
    let cookMovedWhileCarrying = false;
    let previousCookPosition = { x: state.staff[0].x, y: state.staff[0].y };

    for (let tick = 0; tick < 500; tick += 1) {
      state = runTick(state, { gameDt: 1, movementDt: 1 });
      const drink = state.serviceItems.find(item => item.id === 'drink');
      const cook = state.staff.find(worker => worker.id === 'cook');
      const waiter = state.staff.find(worker => worker.id === 'waiter');
      if (drink.state === 'preparing') {
        sawPreparing = true;
        expect(drink.stationId).toBe('dispenser');
        expect(drink.serviceTableId).toBe('st1');
        expect(drink.serviceSlotIndex).toBe(0);
      }
      if (drink.state === 'carried') {
        sawCarried = true;
        ...
        expect(drink.stationId).toBeNull();
        expect(drink.serviceTableId).toBe('st1');
        expect(drink.serviceSlotIndex).toBe(0);
        expect(waiter.task?.type).not.toBe('pickup_service_item');
        cookMovedWhileCarrying ||= cook.x !== previousCookPosition.x
          || cook.y !== previousCookPosition.y;
      }
      if (drink.state === 'on_service') {
        sawOnService = true;
        expect(drink.assignedStaffId).toBeNull();
        expect(cook.task).toBeNull();
        expect(cook.carryingServiceItemIds).not.toContain('drink');
        break;
      }
      previousCookPosition = { x: cook.x, y: cook.y };
    }

    expect(sawPreparing).toBe(true);
    expect(sawCarried).toBe(true);
    expect(cookMovedWhileCarrying).toBe(true);
    expect(sawOnService).toBe(true);
  });

  it('keeps a carried drink reservation in the upper slot while a second drink ticks', () => {
    let state = isolatedRunState({
      kitchenStations: [{ id: 'dispenser', equipmentId: null, x: 360, y: 120 }],
      serviceTables: [{ id: 'st1', x: 140, y: 120 }],
      customers: [
        { id: 'carried-customer', state: 'waiting_for_items', tableId: 't1', drinkId: 'water' },
        { id: 'second-customer', state: 'waiting_for_items', tableId: 't2', drinkId: 'water' },
      ],
      staff: [
        {
          id: 'delivery-cook', role: 'cook', skill: 3, morale: 80, x: 600, y: 300,
        ...
        { id: 'second-cook', role: 'cook', skill: 3, morale: 80, x: 600, y: 300, task: null },
      ],
      serviceItems: [
        {
          id: 'carried-drink', kind: 'drink', menuItemId: 'water', customerId: 'carried-customer',
          tableId: 't1', state: 'carried', serviceTableId: 'st1', serviceSlotIndex: 7,
          assignedStaffId: 'delivery-cook', x: 600, y: 300,
        },
        {
          id: 'second-drink', kind: 'drink', menuItemId: 'water', customerId: 'second-customer',
          tableId: 't2', state: 'ordered', serviceTableId: null, serviceSlotIndex: null,
          assignedStaffId: null,
        },
      ],
    });
    let sawSecondReservation = false;

    for (let tick = 0; tick < 20; tick += 1) {
      state = runTick(state, { gameDt: 1, movementDt: 1 });
      const carried = state.serviceItems.find(item => item.id === 'carried-drink');
      const second = state.serviceItems.find(item => item.id === 'second-drink');
      expect(carried.serviceTableId).toBe('st1');
      expect(carried.serviceSlotIndex).toBe(7);
      if (second.serviceSlotIndex != null) {
        sawSecondReservation = true;
        expect(second.serviceTableId).toBe('st1');
        expect(second.serviceSlotIndex).not.toBe(7);
      }
    }

    expect(sawSecondReservation).toBe(true);
  });

  it('pauses and resumes drink work when actors temporarily block every preparation side', () => {
    const blockedPoints = [
      [80, 100], [80, 120], [140, 100], [140, 120],
      [100, 80], [120, 80], [100, 140], [120, 140],
    ];
    let state = isolatedRunState({
      restaurant: { ...isolatedRunState().restaurant, gameTime: 100 },
      ...
      staff: [
        {
          id: 'cook', role: 'cook', skill: 3, morale: 80, x: 80, y: 110,
          task: {
            type: 'prepare_drink', serviceItemId: 'drink', stationId: 'dispenser',
            serviceTableId: 'st1', serviceSlotIndex: 0,
          },
        },
        ...blockedPoints.map(([x, y], index) => ({
          id: `blocker-${index}`, role: 'waiter', skill: 3, morale: 80, x, y,
          navigationGoal: { x, y },
          task: { type: 'take_order', customerId: `missing-${index}` },
        })),
      ],
      serviceItems: [{
        id: 'drink', kind: 'drink', menuItemId: 'water', customerId: 'c1', tableId: 't1',
        state: 'preparing', stationId: 'dispenser', serviceTableId: 'st1', serviceSlotIndex: 0,
        assignedStaffId: 'cook', preparationStartedAt: 0, accumulatedWork: 20, lastProgressAt: 20,
      }],
    });

    state = runTick(state, { gameDt: 1, movementDt: 1 });
    expect(state.serviceItems[0]).toMatchObject({ accumulatedWork: 20, lastProgressAt: 101 });
    expect(state.serviceItems[0].state).toBe('preparing');

    state = {
      ...state,
      staff: state.staff.map(worker => worker.id === 'cook'
        ? worker : { ...worker, x: 500, y: 500 }),
    };
    let resumed = false;
    for (let tick = 0; tick < 20; tick += 1) {
      state = runTick(state, { gameDt: 1, movementDt: 1 });
      if (state.serviceItems[0].accumulatedWork > 20) {
        resumed = true;
        break;
      }
    }
    ...
    expect(state.serviceItems[0].accumulatedWork).toBeGreaterThan(20);
  });

  it('releases a drink reservation when its customer leaves during preparation', () => {
    let state = isolatedRunState({
      kitchenStations: [{ id: 'dispenser', equipmentId: null, x: 360, y: 120 }],
      serviceTables: [{ id: 'st1', x: 140, y: 120 }],
      customers: [{
    ...
    for (let tick = 0; tick < 100 && state.serviceItems[0].state !== 'preparing'; tick += 1) {
      state = runTick(state, { gameDt: 1, movementDt: 1 });
    }
    expect(state.serviceItems[0].state).toBe('preparing');
    ...
    state = {
      ...state,
      customers: state.customers.map(customer => customer.id === 'c1'
        ? { ...customer, state: 'leaving' }
        : customer),
    };
    const result = runTick(state, { gameDt: 1, movementDt: 1 });

    expect(result.serviceItems.find(item => item.id === 'drink')).toBeUndefined();
    expect(result.staff[0].task).toBeNull();
    expect(result.staff[0].carryingServiceItemIds || []).not.toContain('drink');
  });

  it('releases a carried drink for cleanup when its customer leaves before counter arrival', () => {
    let state = isolatedRunState({
      kitchenStations: [{ id: 'dispenser', equipmentId: null, x: 360, y: 120 }],
      serviceTables: [{ id: 'st1', x: 140, y: 120 }],
      customers: [{
    ...
    for (let tick = 0; tick < 500 && state.serviceItems[0].state !== 'carried'; tick += 1) {
      state = runTick(state, { gameDt: 1, movementDt: 1 });
    }
    expect(state.serviceItems[0].state).toBe('carried');
    ...
    expect(result.serviceItems[0]).toMatchObject({ state: 'to_clean', assignedStaffId: null });
    expect(result.staff[0].task).toBeNull();
    expect(result.staff[0].carryingServiceItemIds || []).not.toContain('drink');
  });
  ...
  it('reroutes a hydrated stationless drink on the first runTick without remote progress', () => {
    const fresh = createInitialState();
    const saved = {
      ...fresh,
      ...
      serviceItems: [{
        ...
        preparationStartedAt: 10, accumulatedWork: 12, lastProgressAt: 20,
      }],
      staff: [{
        id: 'cook', role: 'cook', morale: 80, x: 600, y: 300,
        schedule: Array.from({ length: 48 }, () => 'work'), effectiveDuty: 'work',
        dutyPhase: 'available', dutyTransitionRequestedAt: null, amenityUse: null,
        ptoSession: null, wellRestedUntil: 0, amenityWaitingSince: null,
    ...
    const hydrated = hydrateState(saved, fresh);
    const rerouted = runTick(hydrated, { gameDt: 0, movementDt: 0 });

    expect(rerouted.serviceItems[0]).toMatchObject({
      id: 'drink', state: 'ordered', stationId: 'dispenser', assignedStaffId: 'cook',
      accumulatedWork: 12, lastProgressAt: 100,
    });
    expect(rerouted.staff[0].task).toMatchObject({
      type: 'prepare_drink', serviceItemId: 'drink', stationId: 'dispenser',
    });
  });

  it('bounds the payment-to-first-motion handoff to the next preparation tick', () => {
    vi.spyOn(Math, 'random').mockReturnValue(1);
    const initial = createInitialState();
    let state = {
      ...initial,
      restaurant: {
        ...initial.restaurant, gameTime: 60, funds: 500, dailyRevenue: 0, totalServed: 0,
      },
      tables: [{ id: 't1', status: 'occupied', seats: 2, x: 200, y: 200 }],
      chairs: [],
      staff: [{
      ...
      customers: [{
        ...
        checkoutPosition: { x: 840, y: 180 }, dishId: 'd1', tableId: 't1', patience: 100,
      }],
    ...
    for (let tick = 0; tick < 30; tick += 1) {
      const before = state.customers.find(customer => customer.id === 'c1');
      state = runTick(state, { gameDt: 2, movementDt: 1 / 30 });
      const payer = state.customers.find(customer => customer.id === 'c1');
      trace.push({
        tick,
        ...
        y: payer?.y,
        state: payer?.state,
        goal: payer?.navigationGoal,
        changed: Boolean(before && payer && (before.x !== payer.x || before.y !== payer.y)),
        completed: state.completedCustomers.length,
        served: state.restaurant.totalServed,
        doorAdmission: state.doorAdmissions?.requests?.c1 || null,
        expansions: state.movementCoordinator.diagnostics.expansionsThisTick,
      });
    }

    const paid = trace.find(row => row.served === 1);
    expect(paid).toBeDefined();
    const goal = trace.find(row => row.goal
      && Number.isFinite(row.goal.x) && Number.isFinite(row.goal.y));
    expect(goal).toMatchObject({
      tick: paid.tick + 1,
      state: 'leaving',
      goal: { x: 993, y: 360 },
      doorAdmission: { doorId: 'door1', sequence: expect.any(Number) },
    });
    const moved = trace.find(row => row.tick > paid.tick && row.changed);
    expect(moved).toBeDefined();
    expect((moved.tick - paid.tick) / 30).toBeLessThan(1);
  ...
  it('starts a paid departure alongside existing door traffic', () => {
      ...
      customers: [
        {
          id: 'c1', state: 'checkout_processing', paymentReady: false,
          cashierStationId: 'cashier1', x: 840, y: 180,
          checkoutPosition: { x: 840, y: 180 }, dishId: 'd1', patience: 100,
        },
        {
          id: 'c2', state: 'leaving', exitPhase: 'to_door', exitDoorId: 'door1',
          x: 950, y: 360, navigationGoal: { x: 993, y: 360 },
        },
        {
          id: 'c3', state: 'leaving', exitPhase: 'to_door', exitDoorId: 'door1',
    ...
    for (let tick = 0; tick < 240; tick += 1) {
      ...
      trace.push({
        ...
        served: state.restaurant.totalServed,
        doorAdmission: state.doorAdmissions?.requests?.c1 || null,
        traffic: state.customers.filter(customer => ['c2', 'c3', 'c4'].includes(customer.id))
          .map(customer => ({ id: customer.id, state: customer.state, exitPhase: customer.exitPhase })),
      });
    }

    const paid = trace.find(row => row.served === 1);
    const firstGoal = trace.find(row => row.goal
      && Number.isFinite(row.goal.x) && Number.isFinite(row.goal.y));
    const moved = trace.find(row => row.tick > paid.tick && row.changed);
    expect(paid).toBeDefined();
  ...
  it('never lets a lower-speed duplicate override a moving descriptor regardless of provenance', () => {
    const movingEntry = {
      character: { id: 'party-1', state: 'entering' },
      speed: 62, ignoredIds: [],
    };
    const stationaryDuplicate = {
      character: { id: 'party-1', state: 'entering' },
      speed: 0, ignoredIds: [], provenance: 'guide',
    };

    const merged = mergeMovementEntries([movingEntry], [stationaryDuplicate]);

    expect(merged).toHaveLength(1);
    expect(merged[0]).toBe(movingEntry);
  });
  ...
  it('emits exactly one entry per actor including stationary actors', () => {
    const entries = mergeMovementEntries([
      ...
      { character: { id: 'moving', state: 'leaving' }, speed: 0, ignoredIds: [] },
      { character: { id: 'guide', state: 'entering' }, speed: 62, ignoredIds: [] },
      { character: { id: 'filler', state: 'eating' }, speed: 0, ignoredIds: [] },
    ]);
  ...
  it('completes one deterministic full-service lifecycle and its independent cleanup path', () => {
    ...
    let state = {
      ...
      tables: initial.tables.map(table => table.id === 't1'
        ? {
            ...table, status: 'occupied',
            diningPartyId: 'lifecycle-party', diningCustomerIds: [customerId],
          }
        : table),
      staff: initial.staff
    ...
    const expectedTip = Math.round(lifecycleDish.price * getTipRate(() => 1) * 100) / 100;
    const expectedRevenue = lifecycleDish.price + expectedTip;
    expect(Math.round(expectedRevenue * 100) / 100).toBe(15.6);
    expect(Math.round(finalState.restaurant.dailyRevenue * 100) / 100).toBe(15.6);
  ...
  it('completes a mixed-affordability party lifecycle with one payment and one review', () => {
    ...
    let state = {
      ...
      tables: initial.tables.map(table => table.id === 't1'
        ? {
            ...table, status: 'occupied',
            diningPartyId: 'mixed-party', diningCustomerIds: ['payer', 'waiter'],
          }
        : table),
      staff: initial.staff
        .filter(staff => [
  ...
  it('wipes once and keeps the table empty through delayed checkout departure', () => {
    ...
    let state = {
      ...
      tables: [{
        ...
        diningPartyId: 'former-party', diningCustomerIds: ['former'],
      }],
      customers: [{
        id: 'former', partyId: 'former-party', state: 'checkout_queued', paymentQueuedAt: 1,
        cashierStationId: null, checkoutPosition: null, paymentReady: false,
        ...
        seatResidency: { schema: 1, phase: 'clear', actorId: 'former', partyId: 'former-party' },
      }],
      serviceItems: [{
    ...
    // No cashier waiter exists yet: the physical table lifecycle must not pay.
    expect(state.restaurant.totalServed).toBe(0);
    expect(state.restaurant.dailyRevenue).toBe(0);
    expect(state.completedCustomers).toEqual([]);
  ...
  it('vacates a seated diner to an existing unstaffed cashier and withholds payment until a waiter is assigned', () => {
    vi.spyOn(Math, 'random').mockReturnValue(1);
    const initial = createInitialState();
    const table = initial.tables[0];
    const chair = initial.chairs.find(candidate => candidate.id === 'ch1');
    const diner = {
      id: 'diner', partyId: 'solo', partySize: 1, state: 'checkout_queued',
      tableId: 't1', chairId: 'ch1', x: chair.x + 10, y: chair.y + 10,
      patience: 5000, happiness: 80,
      paymentQueuedAt: 0, cashierStationId: null, checkoutPosition: null, paymentReady: false,
    };
    let state = {
      ...initial,
      restaurant: { ...initial.restaurant, gameTime: 0, reputation: 3, openHour: 10, closeHour: 22 },
      queue: [], floorDirt: [], unlockedDrinkIds: [],
      dishes: [{ id: 'd1', price: 10 }],
      tables: initial.tables.map(candidate => candidate.id === 't1'
        ? { ...candidate, status: 'occupied', diningPartyId: 'solo', diningCustomerIds: ['diner'] }
        : candidate),
      chairs: initial.chairs,
      customers: [{ ...diner, ...recordSeatResidency(diner, chair, table) }],
      serviceItems: [],
      // Existing physical cashier destination with no matching waiter assigned.
      cashierStations: [{ id: 'cashier1', x: 800, y: 120, w: 80, h: 40, assignedStaffId: 'cashier' }],
      staff: initial.staff
        .filter(member => ['starter-cook', 'starter-waiter', 'starter-janitor'].includes(member.id))
        .map((member, index) => ({
          ...member, x: 80 + index * 40, y: 100, task: null, carryingServiceItemId: null,
        })),
    };

    let sawVacating = false;
    for (let tick = 0; tick < 1200; tick += 1) {
      state = runTick(state, 1 / 60);
      const diner = state.customers.find(customer => customer.id === 'diner');
      if (diner?.state === 'checkout_moving') sawVacating = true;
      if (sawVacating && state.tables[0].status === 'empty') break;
    }

    // Normal movement carried the diner out of the seat and freed the table
    // for cleaning without any payment authority.
    expect(sawVacating).toBe(true);
    expect(state.tables[0].status).toBe('empty');
    expect(state.restaurant.totalServed).toBe(0);
    expect(state.restaurant.dailyRevenue).toBe(0);
    expect(state.completedCustomers).toEqual([]);
    ...
    for (let tick = 0; tick < 1200; tick += 1) {
      state = runTick(state, 1 / 60);
      if (state.restaurant.totalServed > 0) { paid = true; break; }
    }
    expect(paid).toBe(true);
  });
...
describe('queue overflow through the merged production pipeline', () => {
  it('keeps guiding and admitting parties with a hidden backlog, without invariants or false unreachable groups', () => {
    ...
    for (let tick = 0; tick < 10; tick += 1) {
      ...
      const stationaryQueueIds = new Set([...coordinator.requests]
        ...
        .map(([id]) => id));
      // Admitted members leave the queue and become moving customers before the
      // batch, so the stationary queue set is the visible set minus admissions.
      for (const id of stationaryQueueIds) {
        expect(expectedVisible.has(id)).toBe(true);
    ...
    // Simulation-owned admission materialised at least one party and reserved
    // its table without any waiter guide task.
    expect(state.customers.length).toBeGreaterThan(0);
    expect(state.tables.some(table => table.status === 'reserved' || table.status === 'occupied'))
      .toBe(true);
    expect(state.staff.every(worker => worker.task?.type !== 'guide_customer')).toBe(true);
  ...
  it('never lets a new arrival coincide with an uncleared queue-origin leaver during staging', () => {
    ...
    expect(state.customers.find(customer => newIds.includes(customer.id)).state).toBe('entering');
...
// Every physical pair stays strictly >= 16: no guide formation exemption remains.
...
describe('queue-lease occupancy is never stolen by a coordinate-less actor', () => {
  function coordinateLessFixture() {
    ...
    return {
      ...
      queueAdmissionGate: null,
      customers: [{
        id: 'coordinate-less', partyId: 'coordless-party', state: 'waiting_for_items',
        tableId: 'queue-table', dishId: null, drinkId: null, chairId: null,
        patience: 100, happiness: 80,
      }],
      staff: [],
      tables: fixture.tables.map(table => ({ ...table, status: 'dirty' })),
      chairs: fixture.chairs,
  ...
  it('merged production ticks keep the standing lease and never materialise the actor', () => {
    vi.spyOn(Math, 'random').mockReturnValue(1);
    let state = coordinateLessFixture();
    for (let tick = 0; tick < 2; tick += 1) {
      state = runTick(state, { gameDt: 0, movementDt: 0.1 });
      expect(state.movementCoordinator.diagnostics.invariantFailure).toBeUndefined();
      ...
      const coordless = state.customers.find(customer => customer.id === 'coordinate-less');
      expect(coordless).toBeDefined();
      expect(Number.isFinite(coordless.y)).toBe(false);
    }
  });
  ...
  it('the hydrated fixture keeps queue spacing through the two preparation phases', () => {
    const fixture = coordinateLessFixture();
    saveState(fixture);
