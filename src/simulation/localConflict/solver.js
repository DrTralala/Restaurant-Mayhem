...
function measureMovementSolverPhase(metrics, key, operation) {
  if (!metrics) return operation();
  const startedAt = solverNow();
  try {
    return operation();
...
function planInitialActors({
  ...
  for (const id of orderedIds) {
    ...
    const plan = planActor({
      ...
      higherActors, plans, ignoreReservations: false, metrics, navigationWorkspace,
    });
    if (!plan) return null;
...
export function solveLocalConflictComponent({
  ...
  const options = {
    ...
    blockedCells,
    horizon,
    progressHorizon: boundedProgressHorizon,
    allowControlledOverlapId,
    initialEdges,
    metrics,
    navigationWorkspace,
...
export function solveLocalConflictWithMovementMetrics(options, metrics = null) {
  const phasesAtStart = metrics
    ? Object.fromEntries(movementSolverPhaseKeys.map(key => [key, metrics[key]]))
    : null;
  const outerAtStart = metrics ? metrics.localConflictSolverMilliseconds : 0;
  const solved = measureMovementSolverPhase(metrics, 'localConflictSolverMilliseconds', () =>
    solveLocalConflictComponent({ ...options, metrics }));
  if (metrics) {
    const outerDelta = metrics.localConflictSolverMilliseconds - outerAtStart;
    const measured = movementSolverPhaseKeys.reduce((total, key) =>
      total + metrics[key] - phasesAtStart[key], 0);
