...
import { getMovementResourceStrategy } from '../movementMetrics';
...
import { moveCharacterTowards } from './pathMotion';
import {
  buildMovementIntent,
  cellsEqual,
...
function resolveConflictComponentAttempt(
  ...
  const prepared = measureLocalConflictPhase(metrics, 'localConflictPreparationMilliseconds', () => {
    ...
    const { workspace = null } = navigation;
    const blockedCells = workspace?.blockedCells ?? buildBlockedCells(state, metrics);
    return {
      actors,
      blockedCells,
      horizon,
  ...
  const selectedId = measureLocalConflictFallbackPhase(metrics, () => {
    for (const intent of component) {
      ...
      intent.hasValidStaticRoute = Boolean(goal
        && findPath(state, worldToCell(intent.character), worldToCell(goal), navigation).length);
    }
...
function resolveCharacterMovementBatchInternal(state, entries, dt, metrics = null) {
  ...
  const workspace = strategy === 'optimised'
    ? createNavigationWorkspace(state, metrics)
    : null;
  const navigation = { workspace, metrics };
