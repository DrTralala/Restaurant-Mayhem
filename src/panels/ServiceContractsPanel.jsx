...
function ContractResult({ result, latest = false }) {
  const template = getServiceContractTemplate(result.templateId);
  return <section style={{ ...cardStyle, borderColor: result.status === 'succeeded' ? '#4a7' : '#b65b61' }}
    aria-label={`${template.title} result ${result.instanceId}`}>
    <h4 style={{ ...TYPOGRAPHY.heading, margin: '0 0 8px' }}>
      {latest ? 'Latest result: ' : ''}{template.title} — {statusLabels[result.status]}
    </h4>
    <p>{result.fulfilledCount}/{result.target} fulfilled meals; {result.guestResults.length} booked guests.</p>
    <p>Bonus ${result.bonusPaid} earned; ${result.reward - result.bonusPaid} not earned.</p>
    <p style={secondaryStyle}>Meal takings are ordinary service revenue, separate from the contract bonus.</p>
    <ul style={{ paddingLeft: 20 }}>{result.guestResults.map((guest, index) =>
      <GuestOutcome key={guest.guestId} guest={guest} label={`${template.guestLabel} ${index + 1}`} />)}</ul>
  </section>;
}
...
export default function ServiceContractsPanel() {
  ...
  const { active, latestResult, results, offers } = selectServiceContractView(state);
  ...
  // This live region changes only for phases, party admission and results, not
  ...
  const announcement = active
    ...
    : latestResult ? `${getServiceContractTemplate(latestResult.templateId).title}: ${statusLabels[latestResult.status]}.` : '';

  return <div style={{ ...TYPOGRAPHY.body, background: '#16213e', color: '#ddd', minWidth: 0 }}>
    <h3 style={{ ...TYPOGRAPHY.heading, color: '#f0a500', margin: '0 0 12px' }}>Service contracts</h3>
    <p>Optional booked services. Guests choose from affordable menu options and may order only drinks.
      Normal walk-ins continue, and accepting does not reserve resources or pause the game.</p>
    <p id="contract-meal-rule" style={secondaryStyle}>A fulfilled meal needs delivered food plus completed checkout
      with a positive dish charge by the deadline. Drinks, cancelled food and unpaid meals do not count.</p>
    {decisionPending && <p style={{ color: '#f0a500' }}>Contract paused — choose Continue in Opening Week to resume</p>}
    <div role="status" aria-live="polite" style={secondaryStyle}>{announcement}</div>
    ...
    {active && <section aria-label="Active contract" style={{ ...cardStyle, borderColor: '#f0a500', marginTop: 12 }}>
      ...
      {active.targetMet && <p style={{ color: '#8cba9d' }}>Target met — bonus settles at {timeLabel(state, active.deadlineAt)}.
        Pending bonus ${active.reward}; no bonus has been paid yet.</p>}
      ...
      {withdrawing === active.instanceId ? <div role="group" aria-label="Confirm contract withdrawal">
        <p>Withdraw and forfeit the bonus? Existing guests remain, and their service continues.
    ...
    {latestResult && <ContractResult result={latestResult} latest />}
    {offers.map(({ template, preview, canAccept, blockedReason, warnings }) => <section key={template.id}
      aria-label={`${template.title} offer`} style={cardStyle}>
      <h4 style={{ ...TYPOGRAPHY.heading, color: '#f0a500', margin: '0 0 8px' }}>{template.title}</h4>
      <p>15 game minutes to prepare · {template.serviceDuration / 60} game minutes of service</p>
      <p>Complete payment for {template.target} fulfilled meals out of {template.parties.reduce((n, party) => n + party.size, 0)} booked guests</p>
      <ol style={{ paddingLeft: 20 }}>{template.parties.map((party, index) => <li key={index} style={{ marginBottom: 6 }}>
        Arrival {index + 1}: {party.partyType}, {party.size} {party.size === 1 ? 'guest' : 'guests'} · {template.profile.archetype}
        {' · '}{template.profile.spendingTier} · ${template.profile.spendingBudget} each · {party.arrivalOffset / 60} game minutes after service starts
      ...
      <p>Bonus ${template.reward}. No entry fee. Failure loses the bonus only; normal service costs and guest reactions still apply.</p>
      {warnings.length > 0 && <div style={secondaryStyle}><strong>Readiness warnings — acceptance is still allowed:</strong>
        <ul style={{ paddingLeft: 20 }}>{warnings.map(warning => <li key={warning}>{warningLabels[warning]}</li>)}</ul>
        <p>These checks are advisory, not a guarantee of seating or service. You can change menu, hours, staff and layout.</p>
      </div>}
    ...
    {results.length > 0 && <details><summary style={{ ...TYPOGRAPHY.control, cursor: 'pointer', marginBottom: 12 }}>
      Result history ({results.length}/10)</summary>
      {[...results].reverse().map(result => <ContractResult key={result.instanceId} result={result} />)}
    </details>}
  </div>;
}
